	...
	*/
	function excluir() {
		if (confirm('Excluir registros selecionados?')) {
			...
			parent.content.document.frm.action = "../<?=DIRETORIO?>/<?=OBJETO?>_excluir.php";
	...
	*/
	function processar() {
		if (confirm('Processar registros selecionados?')) {
			parent.content.document.frm.target = "controle";
			parent.content.document.frm.action = "../<?=DIRETORIO?>/<?=OBJETO?>_processar.php";
			parent.content.document.frm.submit();
		}
	}
	</script>
...
*/
$button->addItem("Novo ".OBJETO_TITULO_SINGULAR,"../".DIRETORIO."/".OBJETO."_edicao.php","content");
$button->addItem("Pesquisa",   "../".DIRETORIO."/".OBJETO."_pesquisa.php","content");
...
while ($rs->getrow()) {
	...
	$table->addData(addLink($rs->field(OBJETO),"../".DIRETORIO."/".OBJETO."_edicao.php?id=$id&pagina=$pg","Clique para consultar ou editar registro"));
	$table->addRow();
}
