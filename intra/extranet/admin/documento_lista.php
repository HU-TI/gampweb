...
if ($iSort) {
	...
	*/
	if ($iSort == 2) setSession("sOrder"," order by documento.titulo" . $sDirection); 
	if ($iSort == 3) setSession("sOrder"," order by documento_categoria.categoria" . $sDirection); 
}
...
if (getParam("rodou")=="s") { // se ocorreu pesquisa...
	...
	*/
	if (getParam("pesq_titulo") != "")    $onde .= "AND documento.titulo LIKE '%" . getParam("pesq_titulo") . "%'";
	if (getParam("pesq_url") != "")       $onde .= "AND documento.arquivo LIKE '%" .    getParam("pesq_url") . "%'";
...
*/
$sql = "SELECT documento.*, documento_categoria.* "
     . " FROM	documento, documento_categoria "
     . " WHERE documento.cod_categoria = documento_categoria.cod_categoria " . getSession("where") . getSession("sOrder");
/*
	...
	*/
	function excluir() {
		if (confirm('Excluir registros selecionados?')) {
			...
			parent.content.document.frm.action = "../admin/documento_excluir.php";
...
*/
$button->addItem("Novo documento","../admin/documento_edicao.php","content");
//$button->addItem("Pesquisa",   "../admin/documento_pesquisa.php","content");
...
$table->addColumnHeader("Titulo",true,"60%", "L"); // Título, Ordenar?, Largura, Alinhamento
$table->addColumnHeader("Categoria",True,"40%", "L"); // Título, Ordenar?, Largura, Alinhamento
$table->addRow(); // adiciona linha (TR)
while ($rs->getrow()) {
	$id = $rs->field("cod_documento"); // captura a chave primária do recordset
	
	$table->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\">");
	$table->addData(addLink($rs->field("titulo"),"../admin/documento_edicao.php?id=$id&pagina=$pg","Clique para consultar ou editar registro"));
	$table->addData($rs->field("categoria"));
	$table->addRow();
}
