...
if ($iSort) {
	...
	*/
	if ($iSort == 2) setSession("sOrder"," order by documento_categoria.categoria" . $sDirection); 
}
...
<html>
<head>
	<title>documento-categoria-lista</title>
	...
	*/
	function excluir() {
		if (confirm('Excluir registros selecionados?')) {
			...
			parent.content.document.frm.action = "../admin/documento_categoria_excluir.php";
	...
	*/
	function processar() {
		if (confirm('Processar registros selecionados?')) {
			parent.content.document.frm.target = "controle";
			parent.content.document.frm.action = "../admin/documento_categoria_processar.php";
			parent.content.document.frm.submit();
		}
	}
	</script>
...
<?
pageTitle("Documentos - Categoria","Lista");
...
*/
$button->addItem("Nova categoria","../admin/documento_categoria_edicao.php","content");
...
$table->addColumnHeader("Categoria",true,"100%", "L"); // Título, Ordenar?, Largura, Alinhamento
...
while ($rs->getrow()) {
	...
	$table->addData(addLink($rs->field("categoria"),"../admin/documento_categoria_edicao.php?id=$id&pagina=$pg","Clique para consultar ou editar registro"));
	$table->addRow();
}
